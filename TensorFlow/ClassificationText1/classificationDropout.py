...
def compute_accuracy(v_xs, v_ys):
    ...
    result = sess.run(accuracy, feed_dict={x_in: v_xs, y_in: v_ys, keep_prob: 1})
    return result
...
l1 = add_layer(x_in, 28 * 28, 110, activation_function = tf.nn.tanh)
#l2 = add_layer(l1, 100, 100, activation_function = tf.nn.sigmoid)
prediction = add_layer(l1, 110, 10, activation_function = tf.nn.softmax)
...
for i in range(10000):
    ...
    sess.run(train_step, feed_dict={x_in: batch_xs, y_in: batch_ys, keep_prob: 0.9})
    if i % 50 == 0:
